...
async function checkIfApprovedCaller() {
  const approvedCallersAbi = [
    "function approvedCallers(address) public view returns (bool)",
  ];
  const contractWithRead = new ethers.Contract(
    CONTRACT_ADDRESS,
    approvedCallersAbi,
    provider
  );
  ...
  if (!isApproved) {
    console.error(
      `Fehler: Die Server-Adresse (${wallet.address}) ist KEIN ApprovedCaller!`
    );
    process.exit(1);
  } else {
    console.log(
      `Die Server-Adresse (${wallet.address}) ist ein ApprovedCaller.`
    );
  }
}
...
// Reward an Studenten vergeben, Testroute
app.post("/reward", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .send("Fehlende Parameter: student, courseId, lessonId oder level.");
  }

  try {
    console.log(
      `Belohnung für Kurs ${courseId} - Lektion ${lessonId} - Level ${level}...`
    );

    // Token vergeben
    const txReward = await contract.rewardStudent(
      student,
      courseId,
      lessonId,
      level
...
app.post("/isCompleted", async (req, res) => {
  ...
  if (
    ...
    return res
      .status(400)
      .send(
        "Fehlende Parameter: student, courseId, lessonId, isLesson oder level."
      );
  }

  try {
    const completed = await contract.isCompleted(
      student,
      courseId,
      lessonId,
      isLesson,
